...
//#define MPTCP_HAS_MASTER(mptcpd) ( (mptcpd)->master )
...
/* Display both 32 and 64 bits forms */
#define PROTO_TREE_ADD_BOTH(tree,hf_field, tvb, offset, length,dsn, additionnal_fmt) \
    proto_tree_add_uint64_format_value( (tree), (hf_field), (tvb) , (offset), (length), (dsn), "%lu  (64bits version)" additionnal_fmt , dsn); \
    proto_tree_add_uint64_format_value( (tree), (hf_field), (tvb) , (offset), (length), (guint32)(dsn), "%u  (32bits version)" additionnal_fmt , (guint32)dsn );
...
static int hf_tcp_option_mptcp_A_flag = -1;
static int hf_tcp_option_mptcp_stream = -1;
static int hf_tcp_option_mptcp_has_master = -1;
...
static int hf_tcp_option_mptcp_expected_idsn = -1;
static int hf_tcp_option_mptcp_subflow_id = -1;
...
//static expert_field ei_mptcp_analysis_reset_recvkey = EI_INIT;
...
static
guint64 dsn_trunc_msb32(guint64 dsn) {
    return ((dsn >> 32) << 32);
}
//static
//guint64 dsn_trunc_lsb32(guint64 dsn) {
//    return (guint32)dsn;
//}
...
//    printf("dsn before %lu\n",dsn);
    DISSECTOR_ASSERT(dsn >> 32 == 0);   //! check 32 MSB are set to 0
//    printf("dsn after %lu\n",dsn);
//    if()
    //TODO if we wanted to do it correclty
//    upperBits = mptcp_look_for_upper_dsn_bits(pinfo,mptcpd, );
//    dsn_trunc_msb32(mptcpd->abs_dsn);
//    mptcpd->dsn= ;
    return dsn_trunc_msb32(f->expected_idsn) | dsn;
}
...
#define MPTCP_META_HAS_SERVERKEY       0x0020
...
//struct tcp_analysis *
gboolean
mptcp_has_master( struct tcp_analysis *tcpd) {
    return (tcpd->mptcp_analysis->mp_flags & (MPTCP_META_HAS_CLIENTKEY | MPTCP_META_HAS_SERVERKEY) );
}
...
////    }
//    // compare dsn rather ?
//    if(m1->ssn < m2->ssn)   return -1;
//    else if (m1->ssn == m2->ssn)    return 0;
//    else return 1;
//}
...
/*
find mapping associated with ssn
*/
...
mptcp_init_flow(mptcp_flow_t *flow ) {
    ...
    flow->flags =0;
    flow->version=0;
//    flow->checksum_required=FALSE;
    flow->token = 0;
//        flow->idsn = 0;
    flow->base_dsn = 0;
    flow->nextseq = 0;
    flow->dsn_wraps = wmem_tree_new(wmem_file_scope());
}
...
*/
static void
mptcp_attach_connection(struct mptcp_analysis* mptcpd, struct tcp_analysis* subflow) {
    // ->stream
...
// TODO rename into flags
    ...
    mptcp_init_flow( &mptcpd->meta_flow1 );
    mptcp_init_flow( &mptcpd->meta_flow2 );

    mptcpd->master=NULL;   /* no master */
    //tcpd->stream;
...
//    }
    ...
    if(&tcpd->flow1 == tcpd->fwd){
        mptcpd->fwd = &(mptcpd->meta_flow1);
        mptcpd->rev = &(mptcpd->meta_flow2);
    }
    else {
        mptcpd->fwd = &(mptcpd->meta_flow2);
        mptcpd->rev = &(mptcpd->meta_flow1);
    }
...
//    guint64 token = user_data
    ...
    /* check key master */
    if(tcpd && is_mptcp(tcpd) && mptcp_has_master(tcpd)) {

        struct mptcp_analysis* mptcpd = tcpd->mptcp_analysis;
//            printf("Comparing token: %u with %u and %u \n",token_to_find,mptcpd->token1,mptcpd->token2);
        return (mptcpd->meta_flow1.token == token_to_find || mptcpd->meta_flow2.token == token_to_find);
    }
...
{
    guint64 key_to_find = *(guint64*)user_data;

    conversation_t* conv = (conversation_t*)value;
    struct tcp_analysis* analysis = (struct tcp_analysis*)conversation_get_proto_data(conv,proto_tcp);

    if(analysis && is_mptcp(analysis) && mptcp_has_master(analysis)) {

        struct mptcp_analysis* mptcpd = analysis->mptcp_analysis;
//            printf("Comparing token: %u with %u and %u \n",token_to_find,mptcpd->token1,mptcpd->token2);
        return (mptcpd->meta_flow1.key == key_to_find || mptcpd->meta_flow2.key == key_to_find);
    }
...
The initial data sequence number on an MPTCP connection is generated
   from the key.
this is costly so don't do it too often .
                Generalize IDSN and token
                Move it to finalize
                Only supports Sha1 for now */
static
gboolean
mptcp_cache_cryptodata(mptcp_hmac_algorithm_t algo _U_,
//const guint8* key,
const guint64 key,
...
//    int i = 0;
    ...
    tvbuff_t *tvb;
    guint64 pseudokey = GUINT64_TO_LE(key);
...
//    for(i = 0; i < ; ++i) {
//    i = 3;
    // TODO unallocate tvb or done automatically ?
    tvb = tvb_new_real_data( (const guint8*)&digest_buf,SHA1_BUFFER_SIZE,SHA1_BUFFER_SIZE);
//    ()
//    *token = tvb_get_ntohl(tvb,SHA1_BUFFER_SIZE-4);
//    printf("result: %u\n",*token);
    // THis one is good
...
/* TODO make sure that the other end confirmed the MP_CAPABLE option
else drop the mptcpd
return false if fallback to TCP ?
...
*/
static gboolean
finalize_mptcp_connection(struct tcp_analysis* tcpd) {
    struct mptcp_analysis *mptcpd = tcpd->mptcp_analysis;
    guint8 res;
    printf("finalizing connection\n");
//    mptcp_hmac_algorithm_t algo;
//    #define GET_MPTCP_VERSION(flag) flags & 0x0F
    #define GET_CHECKSUM(flow) ( (flow.flags) & 0x80)
    #define GET_HMAC_ALGOS(flow) ( (flow.flags) & 0x0F)
    ...
    //!
    mptcpd->version = MIN(mptcpd->meta_flow1.version, mptcpd->meta_flow2.version);
    /*if either host requires the use of checksums, checksums MUST be used*/
    if(GET_CHECKSUM(mptcpd->meta_flow1) || GET_CHECKSUM(mptcpd->meta_flow2)){
      mptcpd->mp_flags |= MPTCP_META_CHECKSUM_REQUIRED;
    }

    res = GET_HMAC_ALGOS(mptcpd->meta_flow1) & GET_HMAC_ALGOS(mptcpd->meta_flow2);

    printf("Algo chosen %u\n", res);
    ...
    mptcpd->state = MPTCP_CON_OK;

    // controversial
    mptcpd->meta_flow1.base_dsn = mptcpd->meta_flow1.expected_idsn;
    mptcpd->meta_flow2.base_dsn = mptcpd->meta_flow2.expected_idsn;
    //! hmac_algo
//    algo = reinterpret_cast<mptcp_hmac_algorithm_t>(algo);
    /* The initiator creates a proposal setting a bit for each algorithm it supports to 1
The responder responds with only 1 bit set -- this is the chosen algorithm */
    //! Generate the token now (need str)

    mptcpd->master=tcpd;
    printf("finalizing connection 2, master %u\n",mptcpd->master->stream);
    #undef GET_CHECKSUM
    #undef GET_HMAC_ALGOS
...
static void
mptcp_analyze(
packet_info *pinfo _U_, tvbuff_t *tvb _U_, proto_tree *tree _U_,
            ...
            )
{
    ...
    if(tcph->th_flags & TH_SYN) {
        ...
        /* if there is an option MP_CAPABLE */
        if(mph->mh_mpc) {

            mptcpd->fwd->flags = mph->mh_flags;
            mptcpd->fwd->key = mph->mh_key;


            /* need to generate expected idsn/token to display straightaway
            It would be possible to check if keys changed */
            if(!mptcp_cache_cryptodata( MPTCP_HMAC_SHA1, mptcpd->fwd->key , &mptcpd->fwd->token, & mptcpd->fwd->expected_idsn)) {
                // TODO
    //            expert_add_info(pinfo, item, &ei_mptcp_analysis_unsupported_algorithm);
            }

            printf("setting mp_flags %u\n",tcph->th_flags);
            mptcpd->mp_flags |= (tcph->th_flags == TH_SYN) ? MPTCP_META_HAS_CLIENTKEY : MPTCP_META_HAS_SERVERKEY;
            mptcpd->master = tcpd;
        }

        if( (tcph->th_flags & (TH_SYN | TH_ACK) )== (TH_SYN | TH_ACK) ) {
            printf("mp_flags before call to finalize: %u\n",tcph->th_flags);
            finalize_mptcp_connection(tcpd);
        }
    }
...
//      }
//      /* or SYN/ACK */
//      else if(tcph->th_flags & (TH_SYN |TH_ACK) == tcph->th_flags) {
//        mptcpd->mp_flags |= MPTCP_META_HAS_SERVERKEY;
//        mptcpd->fwd->key = mph->mh_key;
//
//      }
                //mptcpd->key1
...
//                        base64_to_tvb()
     ...
     */
    if(!mptcp_has_master(tcpd) && (mph->mh_join || mph->mh_fastclose ) ) {
...
//    guint64 dsn = 0;
    guint32 rel_ssn = tcph->th_seq;
//    guint32 rel_ssn = tcph->th_seq;
//    guint16 flags = tcph->flags;
//    guint16 tcpflags = tcph->th_flags;
    proto_item *item;
    proto_tree *tree;
...
//    tcph->th_mptcpstream = tcpd->mptcp_analysis->stream;
    ...
    if( mptcp_has_master(tcpd) &&  ( mptcpd->master != tcpd) ) {

        item = proto_tree_add_boolean_format_value(tree, hf_tcp_option_mptcp_has_master, tvb, 0,
                                     0, FALSE
...
//    else
//      if(mptcp_has_master(tcpd) )
//      {
//
//    }
    else {
          item = proto_tree_add_boolean(tree, hf_tcp_option_mptcp_has_master, tvb, 0,
                                     0, mptcp_has_master(tcpd) ? TRUE : FALSE);
    }

    PROTO_ITEM_SET_GENERATED(item);

    // TODO keep it into mptcp tree
    item = proto_tree_add_uint(tree, hf_tcp_option_mptcp_stream, tvb, 0, 0, mptcpd->stream);
    PROTO_ITEM_SET_GENERATED(item);

//        if(is_mptcp(mptcpd)) {
//            // TODO map seq to DSN
//            //
//            guint64 dsn;  //!< value to save to
    if(!tcp_analyze_seq)
        return ;
    ...
    if(mptcpd->state != MPTCP_CON_OK)
        return ;
            ...
            /* correct seq */
    // 1st need to fix the seq
    // need to recover relative ssn to find mapping
    if(!tcp_relative_seq){
        rel_ssn -= tcpd->fwd->base_seq;
    }
    ...
    if( (mph->mh_dss == TRUE)){
        ...
        if( (mph->mh_flags & (DssFlag_MappingPresent) ) )
        {
            ...
            guint64 abs_dsn=mph->mh_rawdsn;

            /* if dsn of 32bits get abs dsn */
            if( !(DssFlag_DsnOfEightBytes & mph->mh_flags)) {
                abs_dsn=get_full_dsn(pinfo, mptcpd->fwd, abs_dsn);
            }

            if(tcp_relative_seq) {
                guint64 rel_dsn = abs_dsn - mptcpd->fwd->base_dsn;
                item = proto_tree_add_uint64_format_value(tree, hf_tcp_option_mptcp_data_seq_no, tvb, 0, 0, rel_dsn,  "%lu relative DSN mapping)", rel_dsn);
            }
            else {
                item = proto_tree_add_uint64_format_value(tree, hf_tcp_option_mptcp_data_seq_no, tvb, 0, 0, abs_dsn, "%lu", abs_dsn);
            }
...
//                mapping->ssn = mph->mh_ssn;
                ...
                ret = mptcp_subflow_add_mapping(mptcpd->fwd,tcpd->fwd->mptcp,mapping);

//                    else just find the mapping {}
                if(ret == mapping) {
                    // everything fine
                }
// TODO check for infinite mapping ?
//                        else if(*ret == *mapping){
// Raather mappings overlap
//                            expert_add_info(pinfo, mptcp_tree, &ei_mptcp_analysis_duplicate_mapping);
//                        }
                else {
                    expert_add_info(pinfo, tree, &ei_mptcp_analysis_overlap_mapping);
//                    printf(" ===> Mapping %u -> [ %lu - %lu ] overlaps with registered mapping %u -> [ %lu - %lu ]!!\n",mapping->ssn,mapping->abs_dsn,mapping->abs_dsn+mapping->length,ret->ssn,ret->abs_dsn,ret->abs_dsn+ret->length);
                }
        ...
        /* if dack present */
        if( (mph->mh_flags & (DssFlag_DataAckPresent) ) ) {

            guint64 abs_dack = mph->mh_rawack;

            /* if dsn of 32bits get abs dsn */
            if( !(DssFlag_DataAckOfEightBytes & mph->mh_flags)) {
                abs_dack = get_full_dsn(pinfo, mptcpd->rev, abs_dack);
            }

//        dataAck -= mptcpd->rev->base_dsn;
            if(tcp_relative_seq) {
                guint64 rel_dack = abs_dack - mptcpd->rev->base_dsn;
                item = proto_tree_add_uint64_format_value(tree, hf_tcp_option_mptcp_data_ack, tvb, 0, 0, rel_dack ,  "%lu relative DACK)", rel_dack );
            }
            else {
...
//                if(flags & DssFlag_DsnOfEightBytes)
                guint64 rel_dsn = abs_dsn - mptcpd->fwd->base_dsn;
//                printf(" Base seq : %lu", mptcpd->fwd->base_dsn);
    //printf("TOTO tcp falgs %d\n",tcph->th_flags);
    // TODO use
                proto_tree_add_uint64_format_value(tree, hf_tcp_option_mptcp_tcp_seq_to_dsn, tvb, 0, 0, rel_dsn,"%lu (Relative seq)",rel_dsn);
            }
            else {
    //            printf("TOTO 3 tcp falgs %d\n",tcph->th_flags);
    // TODO fix offset rename into analysis
                item =
                 proto_tree_add_uint64(tree, hf_tcp_option_mptcp_tcp_seq_to_dsn, tvb, 0,0, abs_dsn );
...
//            if( GT_ !mptcpd->fwd->nextseq)
  ...
  /* print expected idsn (64 and 32 bits version) etc.. */
  if(mph->mh_mpc) {

      item = proto_tree_add_uint(tree,
              hf_tcp_option_mptcp_expected_token, tvb, 0, 0, mptcpd->fwd->token);
      PROTO_ITEM_SET_GENERATED(item);

      PROTO_TREE_ADD_BOTH(tree,
              hf_tcp_option_mptcp_expected_idsn, tvb, 0, 0, mptcpd->fwd->expected_idsn,"" );
  }
...
//
//static void
//mptcp_print_mp_capable_analysis(packet_info *pinfo, tvbuff_t *tvb, proto_tree *parent_tree,
//                          struct tcp_analysis *tcpd,struct mptcp_analysis *mptcpd)
//{
////         TODO move that to analysis tree as well
...
//              hf_tcp_option_mptcp_expected_idsn, tvb, offset, 0, mptcpd->fwd->expected_idsn + 1,"" );
//
//}
...
//        tcph->th_mptcp->mh_dss = FALSE;
    ...
    mptcp_tree = proto_tree_add_subtree(opt_tree, tvb, offset, optlen, ett_tcp_option_mptcp, &main_item, "Multipath TCP");
...
//            if(tcph->th_flags)
            if(tcp_analyze_mptcp) {
              mptcpd->fwd->version = tvb_get_bits8(tvb,offset,4);
//              MPTCP_META_HAS_CLIENTKEY
//                MIN(tvb_get_bits8(tvb,offset,4), mptcpd->version );
            }
            proto_tree_add_item(mptcp_tree, hf_tcp_option_mptcp_version, tvb,
                        offset, 1, ENC_BIG_ENDIAN);
            offset += 1;

            mph->mh_flags = tvb_get_guint8(tvb, offset);
//            if(tcp_analyze_mptcp) {
//                 = flags;
//            }

            item = proto_tree_add_uint(mptcp_tree, hf_tcp_option_mptcp_flags, tvb,
                        offset, 1, mph->mh_flags);
            ...
            /* hmac_algo */
//            hmac_algo=tvb_get_bit
            /*A crypto algorithm MUST be specified.  If flag bits C through H are
   all 0, the MP_CAPABLE option MUST be treated as invalid */
//            tvb_get_bits8(tvb,offset)
...
//                if(tcp_analyze_mptcp) {
//                    guint64 key = tvb_get_ntoh64(tvb,offset);
//
//                    if(mptcpd->mp_flags & )
...
//                    guint32 token = 0;
                    ...
                    item = proto_tree_add_item(mptcp_tree,
                            ...
                            4, ENC_BIG_ENDIAN);
                    mph->mh_token = tvb_get_ntohl(tvb,offset);
                    offset += 4;
...
//            mptcp_mapping_t mapping;
            ...
            item = proto_tree_add_uint(mptcp_tree, hf_tcp_option_mptcp_flags, tvb,
                            offset, 1, flags);
            mptcp_flags_tree = proto_item_add_subtree(item, ett_tcp_option_mptcp);
            ...
            */
            if (flags & DssFlag_DataAckPresent) {
                guint8 fieldLength = 4;
                guint64 dataAck = 0;
//                char format[] = "%lu  (relative DSN mapping)";
                // TODO this is the "as is" version. Create another hf with 64 bits corrected version
                /* 64bits ack */
                if (flags & DssFlag_DataAckOfEightBytes) {

                    dataAck=tvb_get_ntoh64(tvb,offset);
                    fieldLength = 8;
                    proto_tree_add_uint64_format_value(mptcp_tree, hf_tcp_option_mptcp_data_ack_raw, tvb, offset, fieldLength, dataAck , "%lu (64bits)",dataAck);
                }
                /* 32bits ack */
                else {
                    dataAck = tvb_get_ntohl(tvb,offset);
                    fieldLength = 4;
                    proto_tree_add_uint64_format_value(mptcp_tree, hf_tcp_option_mptcp_data_ack_raw, tvb, offset, fieldLength, dataAck, "%lu (32bits)",dataAck );
                }

                // TODO should go elsewhere; in analysis
//                item = proto_tree_add_uint64_format_value(mptcp_tree, hf_tcp_option_mptcp_data_ack, tvb, offset, fieldLength, dataAck, "%lu (64bits)",dataAck );
//                PROTO_ITEM_SET_GENERATED(item);

                /* TODO complement with 64 bits ? or do it later */
                mph->mh_rawack = dataAck;
                offset += fieldLength;
            }
...
//                mptcp_mapping_t *mapping = wmem_new0(wmem_file_scope(),mptcp_mapping_t);
                ...
                if (flags & DssFlag_DsnOfEightBytes) {
                    ...
                    fieldLength = 8;
                    proto_tree_add_uint64_format_value(mptcp_tree, hf_tcp_option_mptcp_data_seq_no_raw, tvb, offset, fieldLength, dsn,  "%lu  (64bits version)", dsn);
                } else {
                    dsn = tvb_get_ntohl(tvb,offset);
                    fieldLength = 4;
                    proto_tree_add_uint64_format_value(mptcp_tree, hf_tcp_option_mptcp_data_seq_no_raw, tvb, offset, fieldLength, dsn,  "%lu  (32bits version)", dsn);
                }
                mph->mh_rawdsn = dsn;
                offset += fieldLength;
...
//                mapping->ssn =
                ...
                proto_tree_add_item(mptcp_tree,
                            ...
                            2, ENC_BIG_ENDIAN);
                mph->mh_length = tvb_get_ntohs(tvb,offset);
                offset += 2;
    ...
    tcph->th_mptcp->mh_stream = mptcpd->stream;
}
...
//static const char*
//mptcp_algo_to_str(const mptcp_hmac_algorithm_t algo) {
//    static const char algoStr[][5] = { "HMAC" };
//    return algoStr[algo];
//}
...
proto_register_tcp(void)
{
    static hf_register_info hf[] = {
        ...
        { "Multipath TCP Stream index",       "mptcp.stream", FT_UINT32, BASE_DEC, NULL, 0x0,
            NULL, HFILL }},

        { &hf_tcp_option_mptcp_has_master,
          { "Is this a master flow", "tcp.options.mptcp.master", FT_BOOLEAN, BASE_NONE,
            NULL, 0x0, NULL, HFILL}},

        { &hf_tcp_option_mptcp_pending,
          { "Pending (=unassociated) subflow", "tcp.options.mptcp.pending", FT_BOOLEAN, BASE_NONE,
            NULL, 0x0, NULL, HFILL}},
        ...
        { &hf_tcp_option_mptcp_data_ack_raw,
          { "Multipath TCP Data ACK", "tcp.options.mptcp.rawdataack", FT_UINT64,
            BASE_DEC, NULL, 0x0, NULL, HFILL}},
        ...
        { &hf_tcp_option_mptcp_data_seq_no_raw,
          { "Multipath TCP Data Sequence Number", "tcp.options.mptcp.rawdataseqno", FT_UINT64,
            BASE_DEC, NULL, 0x0, NULL, HFILL}},

        { &hf_tcp_option_mptcp_taildsn,
          { "Multipath TCP tail Data Sequence Number", "tcp.options.mptcp.taildsn", FT_UINT64,
            BASE_DEC, NULL, 0x0, NULL, HFILL}},

        { &hf_tcp_option_mptcp_tailssn,
          { "Multipath TCP tail Subflow Sequence Number", "tcp.options.mptcp.taildsn", FT_UINT64,
            BASE_DEC, NULL, 0x0, NULL, HFILL}},
        ...
        { &hf_tcp_option_mptcp_expected_token,
          { "Multipath TCP Subflow token from key", "tcp.options.mptcp.expected_token", FT_UINT32,
            BASE_DEC, NULL, 0x0, NULL, HFILL}},

        { &hf_tcp_option_mptcp_subflow_id,
          { "Multipath TCP Subflow id", "tcp.options.mptcp.subflowid", FT_UINT32,
            BASE_DEC, NULL, 0x0, NULL, HFILL}},

        { &hf_tcp_option_mptcp_expected_idsn,
          { "Multipath TCP Subflow first expected DACK", "tcp.options.mptcp.expected_dack", FT_UINT64,
            BASE_DEC, NULL, 0x0, NULL, HFILL}},

        { &hf_tcp_option_mptcp_tcp_seq_to_dsn,
          { "Multipath TCP SSN to DSN", "tcp.options.mptcp.seq2dsn", FT_UINT64,
            BASE_DEC, NULL, 0x0, NULL, HFILL}},
    ...
    static ei_register_info ei[] = {
        ...
        { &ei_mptcp_analysis_unsupported_algorithm, { "mptcp.analysis.unsupported_algo", PI_PROTOCOL, PI_NOTE, "Unsupported algorithm", EXPFILL }},
    ...
    prefs_register_bool_preference(tcp_module, "analyze_mptcp",
        "Analyze various MPTCP parameters",
